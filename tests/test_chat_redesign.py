...
def test_assistant_reply_with_html_is_escaped_in_the_page_not_executable(client, monkeypatch):
    ...
    assert r.json()["reply"] == dangerous_reply  # JSON-ответ — сырой текст как есть, это не HTML-контекст
    ...
    assert "&lt;img src=x onerror=&#34;alert(1)&#34;&gt;" in page or "&lt;img src=x onerror=&#34;alert(1)&#34;&gt" in page
