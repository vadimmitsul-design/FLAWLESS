...
from app.services.pricing import UsageAmounts, find_price
...
async def chat_completion(alias: str, messages: list[dict], **kwargs: Any):
    ...
    # Keep provider extension fields; LiteLLM validates the OpenAI wire shape.
    return await _router.acompletion(
        model=alias, messages=cast(list[AllMessageValues], messages), **kwargs
    )
