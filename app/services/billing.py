...
from app.services.pricing import UsageAmounts, estimate_call_cost_usd
...
class InsufficientBalance(Exception):
    def __init__(self, balance: Decimal, required: Decimal | None = None):
        ...
        super().__init__(
            f"balance {balance} < required {required}" if required else f"balance {balance} <= 0"
        )
...
def resolve_billing_customer_id(customer: Customer) -> int:
    ...
    return (
        ...
        if customer.is_child and customer.parent_customer_id
        else customer.id
    )
...
async def available_balance(
    ...
    assert reserved is not None  # COALESCE returns zero for an empty aggregate.
...
async def _spent_since(
    ...
    *,
    ...
    assert spent is not None  # COALESCE returns zero for an empty aggregate.
    return spent
...
async def start_call(
    ...
    *,
    ...
    if payer is None:
        raise ValueError(f"customer {billing_customer_id} not found")
...
async def _lock_pending_event(session: AsyncSession, event: UsageEvent) -> bool:
    """Claim a pending settlement without flushing stale ORM state first.

    Every finalizer, including the reaper, locks the event before accounts.
    Reading the database status prevents a stale session from settling twice.
    """
    with session.no_autoflush:
        status = (
            await session.execute(
                select(UsageEvent.status).where(UsageEvent.id == event.id).with_for_update()
            )
        ).scalar_one()
        if status != "pending":
...
async def _lock_accounts(
    ...
    """Acquire every account involved in a settlement in the same ID order."""
    account_ids = {payer_id}
    ...
    with session.no_autoflush:
        rows = (
            (
                await session.execute(
                    select(Customer)
                    .where(Customer.id.in_(account_ids))
    ...
    return accounts[payer_id], accounts.get(author_id) if author_id is not None else None


def _record_usage_charge(
    session: AsyncSession,
    payer: Customer,
    event: UsageEvent,
    charged_rub: Decimal,
...
async def finalize_success(
    ...
    *,
    ...
    if not await _lock_pending_event(session, event):
        payer = await session.get(Customer, event.billing_customer_id, populate_existing=True)
        assert payer is not None  # The event has a nonnullable payer foreign key.
        await session.commit()
        return payer.balance_rub
    payer, author = await _lock_accounts(
        session,
        event.billing_customer_id,
        prompt.author_customer_id if prompt is not None else None,
    )
    ...
    if charged_rub is not None:
        _record_usage_charge(session, payer, event, charged_rub)
    if prompt is not None:
        _apply_prompt_fee(session, payer, author, prompt, event)
    if web_conversation_id is not None:
        if reply_text is None:
            raise ValueError("a web chat settlement requires an assistant reply")
        session.add(
            WebMessage(
...
async def finalize_failure(
    ...
    *,
    ...
    charged_rub = (
        price_in_rub(cost_usd, pricing_cfg)
        if cost_usd is not None and pricing_cfg is not None
        else None
    )
    if usage is not None and charged_rub is not None:
        assert pricing_cfg is not None  # A charge is computed only with this configuration.
        event.input_tokens = usage.input_text_tokens
        event.cached_tokens = usage.cached_tokens
        event.cache_write_tokens = usage.cache_write_tokens
        event.output_tokens = usage.output_tokens
    ...
    if charged_rub is not None:
        payer, _ = await _lock_accounts(session, event.billing_customer_id)
        _record_usage_charge(
            session,
            payer,
            event,
            charged_rub,
...
async def reap_pending_call(session: AsyncSession, event: UsageEvent) -> bool:
    """Release a stale reservation only if no other finalizer has claimed it."""
    if not await _lock_pending_event(session, event):
        await session.commit()
        return False
    event.status = "failed"
    event.error_code = "StaleReservationReaped"
    event.latency_ms = 0
    event.completed_at = utcnow()
    await session.commit()
    return True
...
async def confirm_topup(
    ...
    customer = await session.get(
        Customer, topup.customer_id, with_for_update=True, populate_existing=True
    )
    assert customer is not None  # The top-up has a nonnullable customer foreign key.
    session.add(
        WalletLedger(
...
async def admin_adjust_balance(
    ...
    """
    payer = await session.get(Customer, customer_id, with_for_update=True, populate_existing=True)
    if payer is None:
...
async def purchase_subscription(
    ...
    customer = await session.get(
        Customer, customer_id, with_for_update=True, populate_existing=True
    )
    if customer is None:
        raise ValueError(f"customer {customer_id} not found")
...
async def _lock_paid_order(session: AsyncSession, order: SubscriptionOrder) -> bool:
    """Both order outcomes claim the current order before locking its wallet."""
    with session.no_autoflush:
...
async def fulfill_order(session: AsyncSession, order: SubscriptionOrder, admin_id: int) -> None:
    if not await _lock_paid_order(session, order):
        await session.commit()
        return
...
async def refund_order(session: AsyncSession, order: SubscriptionOrder, admin_id: int) -> Decimal:
    if not await _lock_paid_order(session, order):
        balance = (
            await session.execute(
                select(Customer.balance_rub).where(Customer.id == order.customer_id)
            )
        ).scalar_one()
        await session.commit()
        return balance
    customer = await session.get(
        Customer, order.customer_id, with_for_update=True, populate_existing=True
    )
    assert customer is not None  # The order has a nonnullable customer foreign key.
...
async def charge_prompt_fee(
    ...
    2. РОЯЛТИ ВЫПЛАЧИВАЕТСЯ ТОЛЬКО ИЗ СОБРАННЫХ ДЕНЕГ. Если после списания
       ...
       нет, с записью в лог.
    """
    # Compatibility transaction for callers charging a fee independently.
    # New chat calls include the fee in finalize_success instead.
    event = None
    if usage_event_id is not None:
        event = await session.get(
            UsageEvent, usage_event_id, with_for_update=True, populate_existing=True
        )
        previous_fee = (
            await session.execute(
                select(WalletLedger.id)
                .where(
                    WalletLedger.usage_event_id == usage_event_id,
                    WalletLedger.note == f"prompt #{prompt.id} fee",
                )
                .limit(1)
            )
        ).scalar_one_or_none()
        if previous_fee is not None:
            await session.commit()
            return
    payer, author = await _lock_accounts(session, billing_customer_id, prompt.author_customer_id)
    _apply_prompt_fee(session, payer, author, prompt, event, usage_event_id=usage_event_id)
    await session.commit()


def _apply_prompt_fee(
    session: AsyncSession,
    payer: Customer,
    author: Customer | None,
    prompt: Prompt,
    event: UsageEvent | None,
    *,
    usage_event_id: UUID | None = None,
) -> None:
    """Mutate fee and royalty together, with the caller holding account locks."""
    if author is not None and resolve_billing_customer_id(author) == payer.id:
        return
